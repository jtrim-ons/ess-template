...
export let trimmedIndicators, latestData, primaryIndicators, secondaryIndicators, otherIndicators, selectedAreas, siblingAreas, checkboxedRoles, selectedArea, parentArea;
...
></Key>
    ...
    {#each [...primaryIndicators,...secondaryIndicators].filter((e) => e.includeAsBeeswarm === "T") as indicator, i}
...
{/if}
    ...
    {#if otherIndicators.length >0 }
        ...
        <Twisty label={"Show "+otherIndicators.length+" more indicators"} {open} cross=true>
                
            {#each otherIndicators as indicator}
