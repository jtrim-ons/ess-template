<script>

import { getContext } from 'svelte';
import { AccurateBeeswarm } from 'accurate-beeswarm-plot';
import { scaleLinear } from 'd3-scale';
import { colorsObject } from '$lib/config';
import { select } from "d3-selection";

let areas = getContext('areas'); 

let siblingAreas = [
    "E06000001",
    "E06000002",
    "E06000003",
    "E06000004",
    "E06000005",
    "E06000006",
    "E06000007",
    "E06000008",
    "E06000009",
    "E06000010",
    "E06000011",
    "E06000012",
    "E06000013",
    "E06000014",
    "E06000015",
    "E06000016",
    "E06000017",
    "E06000018",
    "E06000019",
    "E06000020",
    "E06000021",
    "E06000022",
    "E06000023",
    "E06000024",
    "E06000025",
    "E06000026",
    "E06000027",
    "E06000030",
    "E06000031",
    "E06000032",
    "E06000033",
    "E06000034",
    "E06000035",
    "E06000036",
    "E06000037",
    "E06000038",
    "E06000039",
    "E06000040",
    "E06000041",
    "E06000042",
    "E06000043",
    "E06000044",
    "E06000045",
    "E06000046",
    "E06000047",
    "E06000049",
    "E06000050",
    "E06000051",
    "E06000052",
    "E06000053",
    "E06000054",
    "E06000055",
    "E06000056",
    "E06000057",
    "E06000058",
    "E06000059",
    "E06000060",
    "E06000061",
    "E06000062",
    "E07000008",
    "E07000009",
    "E07000010",
    "E07000011",
    "E07000012",
    "E07000026",
    "E07000027",
    "E07000028",
    "E07000029",
    "E07000030",
    "E07000031",
    "E07000032",
    "E07000033",
    "E07000034",
    "E07000035",
    "E07000036",
    "E07000037",
    "E07000038",
    "E07000039",
    "E07000040",
    "E07000041",
    "E07000042",
    "E07000043",
    "E07000044",
    "E07000045",
    "E07000046",
    "E07000047",
    "E07000061",
    "E07000062",
    "E07000063",
    "E07000064",
    "E07000065",
    "E07000066",
    "E07000067",
    "E07000068",
    "E07000069",
    "E07000070",
    "E07000071",
    "E07000072",
    "E07000073",
    "E07000074",
    "E07000075",
    "E07000076",
    "E07000077",
    "E07000078",
    "E07000079",
    "E07000080",
    "E07000081",
    "E07000082",
    "E07000083",
    "E07000084",
    "E07000085",
    "E07000086",
    "E07000087",
    "E07000088",
    "E07000089",
    "E07000090",
    "E07000091",
    "E07000092",
    "E07000093",
    "E07000094",
    "E07000095",
    "E07000096",
    "E07000098",
    "E07000099",
    "E07000102",
    "E07000103",
    "E07000105",
    "E07000106",
    "E07000107",
    "E07000108",
    "E07000109",
    "E07000110",
    "E07000111",
    "E07000112",
    "E07000113",
    "E07000114",
    "E07000115",
    "E07000116",
    "E07000117",
    "E07000118",
    "E07000119",
    "E07000120",
    "E07000121",
    "E07000122",
    "E07000123",
    "E07000124",
    "E07000125",
    "E07000126",
    "E07000127",
    "E07000128",
    "E07000129",
    "E07000130",
    "E07000131",
    "E07000132",
    "E07000133",
    "E07000134",
    "E07000135",
    "E07000136",
    "E07000137",
    "E07000138",
    "E07000139",
    "E07000140",
    "E07000141",
    "E07000142",
    "E07000143",
    "E07000144",
    "E07000145",
    "E07000146",
    "E07000147",
    "E07000148",
    "E07000149",
    "E07000163",
    "E07000164",
    "E07000165",
    "E07000166",
    "E07000167",
    "E07000168",
    "E07000169",
    "E07000170",
    "E07000171",
    "E07000172",
    "E07000173",
    "E07000174",
    "E07000175",
    "E07000176",
    "E07000177",
    "E07000178",
    "E07000179",
    "E07000180",
    "E07000181",
    "E07000187",
    "E07000188",
    "E07000189",
    "E07000192",
    "E07000193",
    "E07000194",
    "E07000195",
    "E07000196",
    "E07000197",
    "E07000198",
    "E07000199",
    "E07000200",
    "E07000202",
    "E07000203",
    "E07000207",
    "E07000208",
    "E07000209",
    "E07000210",
    "E07000211",
    "E07000212",
    "E07000213",
    "E07000214",
    "E07000215",
    "E07000216",
    "E07000217",
    "E07000218",
    "E07000219",
    "E07000220",
    "E07000221",
    "E07000222",
    "E07000223",
    "E07000224",
    "E07000225",
    "E07000226",
    "E07000227",
    "E07000228",
    "E07000229",
    "E07000234",
    "E07000235",
    "E07000236",
    "E07000237",
    "E07000238",
    "E07000239",
    "E07000240",
    "E07000241",
    "E07000242",
    "E07000243",
    "E07000244",
    "E07000245",
    "E07000246",
    "E08000001",
    "E08000002",
    "E08000003",
    "E08000004",
    "E08000005",
    "E08000006",
    "E08000007",
    "E08000008",
    "E08000009",
    "E08000010",
    "E08000011",
    "E08000012",
    "E08000013",
    "E08000014",
    "E08000015",
    "E08000016",
    "E08000017",
    "E08000018",
    "E08000019",
    "E08000021",
    "E08000022",
    "E08000023",
    "E08000024",
    "E08000025",
    "E08000026",
    "E08000027",
    "E08000028",
    "E08000029",
    "E08000030",
    "E08000031",
    "E08000032",
    "E08000033",
    "E08000034",
    "E08000035",
    "E08000036",
    "E08000037",
    "E09000001",
    "E09000002",
    "E09000003",
    "E09000004",
    "E09000005",
    "E09000006",
    "E09000007",
    "E09000008",
    "E09000009",
    "E09000010",
    "E09000011",
    "E09000012",
    "E09000013",
    "E09000014",
    "E09000015",
    "E09000016",
    "E09000017",
    "E09000018",
    "E09000019",
    "E09000020",
    "E09000021",
    "E09000022",
    "E09000023",
    "E09000024",
    "E09000025",
    "E09000026",
    "E09000027",
    "E09000028",
    "E09000029",
    "E09000030",
    "E09000031",
    "E09000032",
    "E09000033",
    "N09000001",
    "N09000002",
    "N09000003",
    "N09000004",
    "N09000005",
    "N09000006",
    "N09000007",
    "N09000008",
    "N09000009",
    "N09000010",
    "N09000011",
    "S12000005",
    "S12000006",
    "S12000008",
    "S12000010",
    "S12000011",
    "S12000013",
    "S12000014",
    "S12000017",
    "S12000018",
    "S12000019",
    "S12000020",
    "S12000021",
    "S12000023",
    "S12000026",
    "S12000027",
    "S12000028",
    "S12000029",
    "S12000030",
    "S12000033",
    "S12000034",
    "S12000035",
    "S12000036",
    "S12000038",
    "S12000039",
    "S12000040",
    "S12000041",
    "S12000042",
    "S12000045",
    "S12000047",
    "S12000048",
    "S12000049",
    "S12000050",
    "W06000001",
    "W06000002",
    "W06000003",
    "W06000004",
    "W06000005",
    "W06000006",
    "W06000008",
    "W06000009",
    "W06000010",
    "W06000011",
    "W06000012",
    "W06000013",
    "W06000014",
    "W06000015",
    "W06000016",
    "W06000018",
    "W06000019",
    "W06000020",
    "W06000021",
    "W06000022",
    "W06000023",
    "W06000024",
];

let plottedAreas = [{"role": "main", "areacd": "E06000002"}];

$: console.log(plottedAreas)
let filteredData = getContext("beeswarmKeyData");
let selectedAreas = [{"data": {"areacd": "E06000002", "areanm": "Middlesbrough"}, "role": "main"}];

let width=100, height=150;

$: outerPadding = {top: 55, left: 40, right: 40, bottom: 65};

$: chartWidth = width - outerPadding.left - outerPadding.right;
$: chartHeight = height - outerPadding.top - outerPadding.bottom;

$: siblingsData = filteredData.filter((e) => siblingAreas.includes(e.areacd) || e.areacd === selectedAreas.find((el) => el.role === "main").data.areacd);

$: console.log(siblingAreas)

function calculateMedian(arr, property) {
  
    const values = arr.map(obj => obj[property]);
    values.sort((a, b) => a - b);
    const length = values.length;

    if (length % 2 === 0) {
        const mid1 = values[length / 2 - 1];
        const mid2 = values[length / 2];
        return (mid1 + mid2) / 2;
    } else {
        return values[Math.floor(length / 2)];
    }
}

function splitTextIntoEqualParts(text, n) {

  const words = text.split(' ');
  const targetLength = Math.ceil(words.length / n);

  const result = [];
  let currentPart = [];

  words.forEach(word => {

    if (currentPart.length < targetLength) {
      currentPart.push(word);
    } else {
      result.push(currentPart.join(' '));
      currentPart = [word];
    }
  });

  if (currentPart.length > 0) {
    result.push(currentPart.join(' '));
  }

  return result;
}

$: medianValue = calculateMedian(siblingsData, "value")

$: maxDistanceFromMedian = Math.max(Math.abs(medianValue - Math.min(...siblingsData.map((e) => e.value))), Math.abs(medianValue - Math.max(...siblingsData.map((e) => e.value))));

$: calculatedDomain = [medianValue - maxDistanceFromMedian , medianValue + maxDistanceFromMedian];

$: siblingsDataCalcRange = siblingsData.map((e) => ({
    ...e,
    calcYValue: (medianValue - e.value)/maxDistanceFromMedian
}))

$: selectedAreasWithData = plottedAreas.filter((e) => e.role === "main").map((e) => ({
    ...e,
    value: filteredData.filter((el) => el.areacd === e.areacd).length > 0 ? filteredData.find((el) => el.areacd === e.areacd).value : null,
    calcYValue: filteredData.filter((el) => el.areacd === e.areacd).length > 0 ? (medianValue - filteredData.find((el) => el.areacd === e.areacd).value)/maxDistanceFromMedian : null
})).filter((e) => e.yValue !== null)


$: yDomain = [-1, 1];

$: y = scaleLinear()
	.domain(yDomain)
	.range([0, chartWidth]);

$: plotData = new AccurateBeeswarm(siblingsDataCalcRange, 6, (d) => y(Math.pow(d.calcYValue, 1)))
        .calculateYPositions();

$: x = scaleLinear()
	.domain([Math.min(...plotData.map(d => d.y)), Math.max(...plotData.map(d => d.y))])
	.range([0, chartHeight]);

$: plotDataPrimary = new AccurateBeeswarm(selectedAreasWithData, 8, (d) => y(Math.pow(d.calcYValue, 1)))
        .calculateYPositions();

$: maxElement = plotData.reduce((prev, curr) => prev.x > curr.x ? prev : curr);

$: explanationText = splitTextIntoEqualParts("Each circle represents a local authority. This is the highest ranking area.", 2)



</script>


<div class="keyContainer">

    <div class="svg-container" bind:clientWidth={width}>

        <svg
        {height}
        {width}
        >

                <g class="chart-container" transform={"translate("+outerPadding.left+","+outerPadding.top+")"}>

                    <g class="medianLineGroup" transform={"translate("+y(0)+",0)"}>

                        <line
                        x1=0
                        x2=0
                        y1=-18
                        y2={chartHeight+20}
                        stroke="#F66068"
                        stroke-width="2.5px"
                        ></line>

                    </g>

                    <g class="backgroundCirclesGroup">

                        {#each plotData as circle,i}
    
                                <g transform={"translate("+circle.x+","+x(circle.y)+")"}>
    
                                    <circle
                                    pointer-events="none"
                                    r=6
                                    stroke="none"
                                    fill="white"
                                    ></circle>
    
                                </g>
    
                        {/each}
    
                    </g>

                    <g class="backgroundCirclesGroup">

                        {#each plotData as circle,i}
    
                                <g transform={"translate("+circle.x+","+x(circle.y)+")"}>
    
                                    <circle
                                    pointer-events="none"
                                    r=5
                                    stroke={circle.datum.areacd === maxElement.datum.areacd ? "black" : "none"}
                                    fill="#6e6e6e"
                                    fill-opacity=0.25
                                    ></circle>
    
                                </g>
    
                        {/each}
    
                    </g>

                    <g class="medianLineGroup" transform={"translate("+y(0)+",0)"}>

                        <text
                        y=-36
                        text-anchor="middle"
                        fill="#F66068"
                        stroke="#F66068"
                        stroke-width="0.25px"
                        font-size="14px">Half of all areas have a value</text>

                        <text
                        y=-22
                        text-anchor="middle"
                        fill="#F66068"
                        stroke="#F66068"
                        stroke-width="0.25px"
                        font-size="14px">higher/lower than this line</text>

                        {#each ["better", "worse"] as word, i}

                            <path
                            transform={"translate("+0+","+(-40)+")"}
                            d="M {-100*[-1,1][i]} 0 l{-50*[-1,1][i]} 0"
                            stroke-width="1px"
                            stroke="black"
                            ></path>

                            <path
                            transform={"translate("+0+","+(-40)+")"}
                            d="M {-158*[-1,1][i]} 0 l{6*[-1,1][i]} -6 l0 12 z"
                            stroke="none"
                            fill="black"
                            ></path>

                            <text
                            transform={"translate("+0+","+(-40)+")"}
                            x={-155*[-1,1][i]}
                            y=15
                            text-anchor="middle"
                            fill="black"
                            font-size="14px">{word} than</text>

                            <text
                            transform={"translate("+0+","+(-40)+")"}
                            x={-155*[-1,1][i]}
                            y=27
                            text-anchor="middle"
                            fill="black"
                            font-size="14px">average</text>


                        {/each}

                    </g>

                    <g class="primaryCirclesGroup">

                        {#each plotDataPrimary as circle,i}
    
                                <g transform={"translate("+y(0.4)+","+x(circle.y)+")"}>
    
                                    <circle
                                    pointer-events="none"
                                    r=6
                                    stroke="white"
                                    stroke-width="1px"
                                    fill={colorsObject[circle.datum.role]}
                                    opacity=1
                                    ></circle>

                                    <g class="labelGroup">

                                        <rect
                                        x=3
                                        y=-20
                                        width=35
                                        height=18.5
                                        rx="3px"
                                        stroke={colorsObject[circle.datum.role]}
                                        fill="white"
                                        stroke-width="1px"
                                        ></rect>

                                        <text
                                        x=6
                                        y=-6
                                        text-anchor="start"
                                        fill={colorsObject[circle.datum.role]}
                                        font-size="14px">{"49%"}</text>

                                    </g>   
    
                                </g>
    
                        {/each}
    
                    </g>

                    <g class="explanationGroup" transform={"translate("+maxElement.x+","+(x(maxElement.y))+")"}>

                        <path
                        transform="translate(0,18)"
                        d="M 0 0 l 0 30 l-20 0"
                        stroke-width="1px"
                        stroke="black"
                        fill="none"
                        ></path>

                        <path
                        transform="translate(0,10)"
                        d="M 0 0 l6 6 l-12 0 z"
                        stroke="none"
                        fill="black"
                        ></path>

                        {#each explanationText as line, i}

                            <text
                            transform={"translate(-22,52)"}
                            text-anchor="end"
                            x=0
                            y={i*14}
                            fill="black"
                            font-size="14px">{line}</text>

                        {/each}

                    </g>

                </g>



        </svg>
    </div>

</div>



<style>

.keyContainer {
    margin: 20px 0px 0px 0px;
    width: 80%;
}

.svg-container {
    margin-bottom: 20px;
    margin-left: 20px;
    border-radius: 15px;
    border-style: solid;
    border-width: 0px;
    border-color: grey;
    background-color: #F5F5F6;
    

}





</style>