<script>
  ...
  let selectedGeogLevel = areasGeogLevel.find(
    (e) => e.areacd === data.areacd
  ).level;
  let areasWithSameGeog = areasGeogLevel
    .filter((e) => {
      if (["uk", "country"].includes(selectedGeogLevel)) {
        return e.level === selectedGeogLevel;
      } else if (selectedGeogLevel === "region") {
        return ["region", "country"].includes(e.level);
      } else if (selectedGeogLevel === "both") {
        return ["both", "lower"].includes(e.level);
      } else {
        return ["both", selectedGeogLevel].includes(e.level);
      }
    })
    .map((e) => e.areacd);
  ...
  let baselineComparisonOptions = [
    "Median of all " + geogLevelToNameLookup[selectedGeogLevel],
  ];
  $: chosenId1 = baselineComparisonOptions[0];
  $: baselineComparisonArea =
    chosenId1.substring(0, 6) === "Median"
      ? {
          areacd: "median",
          areanm: "Median of all " + geogLevelToNameLookup[selectedGeogLevel],
          role: "median",
        }
      : areas.find((e) => e.areanm === chosenId1);
  ...
  if (
    (parentLookup.parentcd != parentLookup.countrycd) &
    (parentLookup.countrycd != "")
  ) {
    selectedAndRelatedAreas.push({
      ...areas.find((e) => e.areacd === parentLookup.countrycd),
      role: "country",
    });
  ...
  if (parentLookup.parentcd != "K02000001") {
    selectedAndRelatedAreas.push({
      ...areas.find((e) => e.areacd === "K02000001"),
      role: "uk",
    });
  ...
  $: visibleAreas = [
    ...selectedAndRelatedAreas.filter((e) => globalVisibleAreasTracker[e.role]),
    ...(globalVisibleAreasTracker.custom == null
      ? []
      : globalVisibleAreasTracker.custom.map((e, i) => ({
          ...e,
          role: "custom" + (i + 1),
        }))),
  ];
  ...
  console.log({ indicatorsCalculations });
  indicatorsCalculations.forEach((e) => {
    let indicator = indicators.find((el) => e.id === el.id);
    let sameGeogLevelData = indicator.data.filter((el) =>
      areasWithSameGeog.includes(el.areacd)
    );
    const TMP_TIME = performance.now();
    ...
    for (var i = e.minXDomain; i <= e.maxXDomain; i++) {
      ...
      let periodDataSameGeog = periodData.filter((el) =>
        areasWithSameGeog.includes(el.areacd)
      );
      ...
      if (periodData.length > 0) {
        if (i == e.maxXDomain) {
          ...
          let deviations = periodDataSameGeog.map((e) =>
            Math.abs(e.value - medianValue)
          );
          let medianAbsoluteDeviation = calculateMedian(deviations);

          e.calcs[e.maxXDomain] = {
            median: medianValue,
    ...
    console.log("TIME", performance.now() - TMP_TIME);
  });
  ...
  let previousTimePeriodOptions = [
    ...["earliest available year", "previous year"],
    ...Array.from(
      { length: globalMainMaxXDomain - globalMainMinXDomain },
      (_, index) => globalMainMinXDomain + index
    ).reverse(),
  ];
  ...
  let customiseCheckbox = [
    "Include description text (e.g. higher/better than median, improving/increasing)",
    "Include background colours",
    "Include better/worse/improving/worsening (rather than higher/lower/increasing/decreasing) wording",
    "Similar to/little change based on being within 2 MADs rather than 1",
    "Rotate change over time axis",
    "Use titles and subtitles directly from metadata",
  ];
  $: chosenId11 = [0, 1, 2];
</script>
...
<Titleblock
  breadcrumb={[
    ...
    { label: "Find a local area", url: `${base}/areas` },
    { label: selectedArea.areanm },
  ]}
>
  <Headline>{selectedArea.areanm}</Headline>
  <Subhead
    >Get localised data, insights and trends for {selectedArea.areanm}</Subhead
  >
</Titleblock>

<SectionsWithNav contentsLabel="Explore this area">
  <div class="temp-container">
    <p class="checkbox-title" style="font-size: 16px">
      The following custom options affect the key indicators and rows of
      indicators sections. These options are for testing different possibilities
      for the final design, rather than options that will be available to users
  ...
  <div class="visual-content-container">
    <KeyIndicators
      indicators={indicators.filter((e) =>
        [
          "annual-personal-wellbeing-estimates-Anxiety mean score (where 10 is 'completely anxious')",
          "female-healthy-life-expectancy",
          "gross-disposable-household-income-per-head",
          "employment-rate-long-Unemployment Rate",
          "aged-16-to-64-years-level-3-or-above-qualifications",
        ].includes(e.code)
      )}
      {chosenId11}
      baselineComparisonArea={selectedAndRelatedAreas.find(
        (e) => e.role === "country"
      )}
      {chosenId2}
    ></KeyIndicators>
    ...
    {#each topics as topic, i}
      <TopicSection
        {topic}
        {visibleAreas}
        bind:globalVisibleAreasTracker
        bind:chosenId1
        {baselineComparisonArea}
        bind:chosenId2
        bind:chosenId4
