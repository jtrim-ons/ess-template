<script>
  ...
  setContext("GLOBAL_DATA", JSON.parse(JSON.stringify(data)));

  let topics = [...new Set(indicators.map((e) => e.topic))]
    .map((e) => ({
      name: e,
      indicatorsList: indicators
        .filter((el) => el.topic === e)
        .map((el) => el.id),
      subTopicsList: [
        ...new Set(
          indicators.filter((el) => el.topic === e).map((el) => el.subTopic)
        ),
      ],
    }))
